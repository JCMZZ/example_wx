...
//					ResponseUtils.outputWithJson(response, Result.badResult("签名校验失败"));
//					return;
//				}
				...
				if (wxUserInfoDTO != null) {
					...
					if (user == null) {
						estoreUserService.saveBusinessUser(wxUserInfoDTO);
					} else {
						// 判断用户是否注册
					...
					ThirdSession thirdSession = new ThirdSession(openId, sessionKey,null);
	...
	public void sendVerificationCode(HttpServletRequest request, HttpServletResponse response) {
		...
		Boolean isExist = estoreAdminService.isExist(mobile);
		if(isExist){
	...
	public void register(HttpServletRequest request, HttpServletResponse response) {
		...
		EstoreAdminDTO estoreAdminDTO = estoreAdminService.findByOpenId(openId);
		if(estoreAdminDTO == null){
			map.put("isLogin", false);
			ResponseUtils.outputWithJson(response, Result.badResult("no login", map));
			return;
		}
...
//		String vcode = "1111";//仅用于外网测试不能实际发送验证码的情况
		...
		if(estoreUser == null){
			// 根据id更新用户手机号
			estoreUserService.updateUserMobile(estoreAdminDTO.getUserId(), mobile);
		} else {
			Boolean isExist = estoreAdminService.isExist(mobile);
			if(isExist){
				ResponseUtils.outputWithJson(response, Result.badResult("该手机号已被注册"));
			...
			//如果手机号已存在，则删除estore_user表中当前用户的信息，并更新estore_admin中user_id
			estoreUserService.deleteById(estoreAdminDTO.getUserId());
			estoreAdminService.updateUserId(estoreUser.getId(), estoreAdminDTO.getUserId());
		}
		ResponseUtils.outputWithJson(response, Result.rightResult());
	}
